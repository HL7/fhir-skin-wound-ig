...
This section outlines foundational Accuracy and Authenticity (A/A) attributes requirements common to all Lower Extremity Skin Wound Assessment acts, events, and actors used in this guide. This FHIR IG is an initial step towards instantiating A/A attributes in FHIR and will evolve from this base. This work is intended to model A/A in a manner that can then be replicated by extension of any clinical content rendered conformant with FHIR.

### Underlying Rationale

At this stage the underlying rationale references U.S. domain legal processes and law along with more universal references in Data Quality. Pending assessment against other legal domains, users may consider this IG to be U.S. centric unless their own assessments prove otherwise.
...
This specification-dependent, "fit for use" dependent approach is incorporated in this FHIR IG in this form: Accuracy, in production, requires an end-use specification that defines all parameters deemed necessary for each end use.

The gradation of "accuracy" then becomes a conformance test, comparing a given data object (purporting to conform to a defined specification) with the stated specification. The specification also stipulates format for the conformance test output and acceptable means used to proveconformance.

These latter statements define, as per U.S. domain legal process and law<sup>[3](#aafootnote3)</sup>, an operational approach to authenticity by modeling conformance testing that answers three questions:
...
The FHIR IG is the selected Research and Development path for modeling Accuracy and Authenticity in accordance with existing and evolving FHIR development processes, to expedite its nascent utility for supporting practical, implementable, verifiable end-to-end trust constructs. The Lower Extremity Skin Wound Assessment and Treatment Template originated within an EHR Systems Workgroup PSS with extensive interest and support from the Podiatry community, including practicing clinicians, EHR vendors, risk insurers, and its professional society. It also references extensive Wound Assessment work resourced from the Agency for Healthcare Research and Quality (AHRQ) and CIMI's work on Wound Assessment for clinical content attributes. The work also benefits from the recent interest and inputs from entities developing specifications for Prior Authorization and for Attachments. (Please see the [Podiatry PSS Confluence page](https://confluence.hl7.org/display/CIC/Skin+and+Wound+PSS) for curated references.)

This FHIR IG begins to address operationalization of a defined, specified, testable requirement for end-to-end trust, which is to provide means for a given end-user to create a specification for data quality that verifiably meets their data definition of "fit for use". It has a number of dependencies, some of which are also evolving in real time, including rigorous adherence to Role-Based Access Controls (RBAC), the normalization of provenance that is verifiably also "fit for use", conformance with rigorous audit function specifications (Ex: ASTM E2147-18), and the future development of nomenclature and naming conventions for a library of end-use defined Accuracy/Authenticity (A/A) Specifications.

<br />

## Ongoing Development

Current participants recognize (and experience) the burdens and harms of data quality assurance failures aka untrustworthy clinical data and records. EHRs, on the whole, have set aside data quality as a systems requirement.  More importantly, those EHRs that have supported data quality assurance support are invisible in the marketplace without test specifications. Those test specifications for data quality constructs require defining and refining use of "Accuracy" and "Authenticity" in terms with sufficient specificity to translate into system design specifications. Those specifications, as in other industries, must be designed for testing as a precondition to continuous improvement. Testable attributes based on known data quality principles and US legal process and law will enable vendors to confidently redirect investment toward supporting accurate data and authentic records. Testability and the resulting consumer transparency are also required to differentiate and evolve the current IT base towards sufficient A/A competency to verifiably meet end-user's "fit for use" requirements. As the research, development, and uptake for this work increasingly connects with others invested in assuring accurate data and authentic records, an energetic and rewarding development path is expected.
...
<p><a name="aafootnote3"><sup>3</sup></a><small>Note the caveat in <a href="#underlying-rationale">Underlying Rationale</a> above, that only U.S. domain references have, so far, been reviewed and referenced to support this operational definition of "authenticity". Within that constraint, this approach has support as noted in footnote <a href="#aafootnote4"><sup>4</sup></a>. U.S.law generally references current knowledge in relevant subject matter areas but research on this point hasn't yet been completed.</small></p>
